  ...
  *)
  ...
  local
    ...
    fun parseArgs' ([], []) = if !reqFile = "" then Help (!submit, NONE)
      ...
      | parseArgs' ("-S" :: args, files) = (submit := true; parseArgs' (args, files))
      | parseArgs' ("--Submit" :: args, files) = (submit := true; parseArgs' (args, files))
      | parseArgs' ("-s"::arg::args, files) = (submit := true;
          setReqFile (arg); parseArgs' (args, files))
      | parseArgs' ("--submit"::arg::args, files) = (submit := true;
          setReqFile (arg); parseArgs' (args, files))
